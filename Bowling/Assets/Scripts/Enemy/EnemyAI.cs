...
public abstract class EnemyAI : MonoBehaviour
{
    ...
    [SerializeField] public float dashTime = 10.0f;     //どれだけ突進するか
    [SerializeField] public float attackRadius = 0.1f; //当たり判定の半径
    ...
    void Attack()
    {
        ...
        if (attackTimer >= attackInterval)
        {
            if (Time.time >= nextAttackRequestTime &&
                EnemyManager.Instance.TryRequestAttack(this))
            {
...
//        RotateToPlayer();
//        UpdateDistanceMovement(distance);
...
//    private void UpdateDistanceMovement(float distance)
//    {
//        if (_isDashing) return;
...
//        if (distance > keepDistance)
//        {
//            desired = toPlayer; // 近づく
//            _attackTimer = 0f;
//        }
//        else if (distance < retreatDistance)
//        {
//            desired = -toPlayer; // 離れる
//            _attackTimer = 0f;
//        }
...
//    {
//        if (!EnemyManager.Instance.TryRequestAttack(this))
//            return;
...
//        _dashDir = (_player.position - transform.position).normalized;
//        _dashDir.y = 0f;

//        Debug.Log($"{name} が突進攻撃開始");
//    }
